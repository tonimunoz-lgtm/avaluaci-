...
// Objecte global per guardar fórmules i arrodoniments
const calculatedActivitiesData = {}; // { [activityId]: { formula: "...", round: true/false } }
    ...
    Promise.all(classActivities.map(id => db.collection('activitats').doc(id).get()))
      .then(actDocs => {
        ...
        actDocs.forEach(adoc => {
          ...
          // 🔴 Icona d'actualització només per activitats calculades
          if (calculatedActs[id]) {
            // Guardem fórmula i arrodoniment a l'objecte global
            calculatedActivitiesData[id] = {
              formula: calculatedActs[id].formula || '', 
              round: calculatedActs[id].round || false
            };

            const refreshBtn = document.createElement('button');
            refreshBtn.innerHTML = '🔄'; // Pots posar un SVG si vols
            refreshBtn.title = 'Actualitzar nota calculada';
            refreshBtn.className = 'ml-2 text-gray-500 hover:text-gray-700';

            refreshBtn.addEventListener('click', () => {
              updateCalculatedCell(id);
            });

            container.appendChild(refreshBtn);

            // Capçalera de color per activitats calculades
            thEl.style.backgroundColor = "#fecaca";
            thEl.style.borderBottom = "3px solid #dc2626";
            thEl.style.color = "black";
          }
...
// Funció per actualitzar només una activitat calculada
function updateCalculatedCell(activityId) {
  const activityData = calculatedActivitiesData[activityId];
  ...
  classStudents.forEach(sid => {
    ...
    const input = row.querySelector(`input[data-activity-id="${activityId}"]`);
    if (!input) return;

    const newVal = computeCalculatedNote(sid, activityId, activityData.formula, activityData.round);
    input.value = newVal;
  });

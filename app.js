...
auth.onAuthStateChanged(user => {
    if (user) { professorUID = user.uid; setupAfterAuth(user); } 
    else { professorUID = null; showLogin(); }
});
...
function loadClassesScreen() {
    ...
    }).catch(e => { classesGrid.innerHTML = '<div class="text-sm text-red-500">Error carregant classes</div>'; console.error(e); });
}
    ...
    classStudents.forEach((stuId, idx)=>{
        db.collection('alumnes').doc(stuId).get().then(doc=>{
            ...
            li.innerHTML = `
                ...
                <div class="relative">
                    <button class="menu-btn text-gray-500 hover:text-gray-700 dark:hover:text-white tooltip">⋮ <span class="tooltiptext">Opcions</span></button>
            ...
            li.querySelector('.edit-btn').addEventListener('click', ()=> {
                ...
                db.collection('alumnes').doc(stuId).update({ nom: newName.trim() }).then(()=> loadClassData()).catch(e=> alert('Error editant alumne: '+e.message));
            });
            ...
            li.addEventListener('drop', e=> { e.preventDefault(); const fromIdx = Number(e.dataTransfer.getData('text/plain')); reorderStudents(fromIdx, idx); });
...
function renderNotesGrid(){
    ...
    const headRow = document.createElement('tr'); headRow.appendChild(th('Alumne'));
    ...
    Promise.all(classActivities.map(id => db.collection('activitats').doc(id).get()))
        .then(actDocs=>{
            actDocs.forEach(adoc=>{
                ...
                container.appendChild(spanName); container.appendChild(menuDiv); thEl.appendChild(container); headRow.appendChild(thEl);
                ...
                menuDiv.querySelector('.edit-btn').addEventListener('click', ()=> {
                    ...
                    if(!newName || newName.trim()===name) return;
                    db.collection('activitats').doc(id).update({ nom: newName.trim() }).then(()=> loadClassData()).catch(e=> alert('Error editant activitat: '+e.message));
                });
            ...
            Promise.all(classStudents.map(id => db.collection('alumnes').doc(id).get()))
                .then(studentDocs=>{
                    studentDocs.forEach(sdoc=>{
                        ...
                        const tdName = document.createElement('td'); tdName.className = 'border px-2 py-1'; tdName.textContent = sdata.nom; tr.appendChild(tdName);
                        ...
                        actDocs.forEach(actDoc=>{
                            ...
                            input.type='number'; input.min=0; input.max=10; input.value=val; input.className='table-input text-center rounded border p-1';
                            input.addEventListener('change', e=> saveNote(sid, aid, e.target.value));
                            input.addEventListener('input', ()=> applyCellColor(input)); applyCellColor(input);
                            td.appendChild(input); tr.appendChild(td);
                        });

                        const avgTd = document.createElement('td'); avgTd.className = 'border px-2 py-1 text-right font-semibold';
                        avgTd.textContent = computeStudentAverageText(sdata); tr.appendChild(avgTd);
...
function saveNote(studentId, activityId, value){
    ...
    db.collection('alumnes').doc(studentId).update(updateObj).then(()=> renderNotesGrid()).catch(e=> console.error('Error saving note', e));
}
...
function renderAverages(){
    ...
    for(let i=0;i<actCount;i++){
        ...
        const td = document.createElement('td'); td.className = 'border px-2 py-1 text-center font-semibold'; td.textContent = avg; tr.appendChild(td);
    }
    tr.appendChild(th('','')); notesTfoot.appendChild(tr);
}
...
document.addEventListener('click', function(e){
    document.querySelectorAll('.menu').forEach(menu=>{
        if(!menu.contains(e.target) && !e.target.classList.contains('menu-btn')) menu.classList.add('hidden');
    });
});

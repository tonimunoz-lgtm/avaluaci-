...
function createStudentModal(){
    ...
    ref.set({ nom: name, notes: {} })
        .then(()=> db.collection('classes').doc(currentClassId).update({
...
function createActivityModal(){
    ...
    ref.set({ nom: name, data: new Date().toISOString().split('T')[0] })
        .then(()=> db.collection('classes').doc(currentClassId).update({
            activitats: firebase.firestore.FieldValue.arrayUnion(ref.id)
        }))
        .then(()=> {
                ...
                .then(studentDocs=>{
                    studentDocs.forEach(sdoc=>{
                        ...
                        actDocs.forEach(actDoc=>{
                            ...
                            input.type='number'; input.min=0; input.max=10; input.value=val;
...
function renderAverages(){
    ...
    for(let i=0;i<actCount;i++){
        ...
        const td = document.createElement('td'); td.className='border px-2 py-1 text-center font-semibold';
        td.textContent = avg;
        tr.appendChild(td);
    }
    tr.appendChild(th('',''));
